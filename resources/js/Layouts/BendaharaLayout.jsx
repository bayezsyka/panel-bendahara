...
export default function BendaharaLayout({ children, header }) {
    ...
    const SidebarDropdown = ({ name, icon, children, activeRoutes }) => {
        const active = isAnyActive(activeRoutes);
        ...
        return (
            <div className="flex flex-col space-y-1">
                <button
                    ...
                    className={`group/link relative flex w-full items-center justify-between px-3 md:px-0 md:pl-[1.15rem] md:pr-2 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ease-in-out overflow-hidden whitespace-nowrap ${active
                ...
                >
                    <div className="flex items-center">
                        <div className={`flex-shrink-0 w-5 h-5 transition-colors duration-200 ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400 group-hover/link:text-indigo-600 dark:group-hover/link:text-indigo-400'}`}>
                            {icon}
                        </div>
                    ...
                    <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'rotate-180' : ''}`}>
                        <svg className={`w-4 h-4 ml-2 text-gray-400 dark:text-gray-500 transition-opacity duration-300 hidden md:block md:opacity-0 md:group-hover:opacity-100`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                        <svg className={`w-4 h-4 ml-2 text-gray-400 dark:text-gray-500 md:hidden`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                    </div>
                </button>
                {isOpen && (
                    <div className="flex flex-col space-y-1 pl-4 md:pl-[0.85rem] border-l-2 border-transparent transition-all duration-300 overflow-hidden hidden md:flex md:w-0 md:group-hover:w-full md:opacity-0 md:group-hover:opacity-100">
                        {children}
                    </div>
                )}
                {isOpen && (
                    <div className="flex flex-col space-y-1 pl-9 border-l-2 border-gray-100 dark:border-gray-700/50 md:hidden transition-all duration-300">
                        {children}
                    </div>
                )}
            </div>
    ...
    const SidebarLink = ({ name, routeName, icon, badge, isSub = false }) => (
        <Link
    ...
    const DarkModeToggle = () => (
        <div className="flex items-center justify-between w-full px-3 py-2.5 rounded-lg">
            ...
            >
                <span
                    className={`inline-flex h-4 w-4 items-center justify-center rounded-full bg-white shadow-lg transform transition-all duration-300 ${isDarkMode ? 'translate-x-6' : 'translate-x-1'
                        }`}
                >
                    ...
                    <div className="flex-1 overflow-y-auto px-3 py-4 custom-scrollbar space-y-1">
                        ...
                        {activePanel === 'finance' && (
                            <>
                                ...
                                <SidebarLink
                                    ...
                                    routeName="projectexpense.projects.trash"
                        ...
                        {activePanel === 'delivery' && (
                            <>
                                ...
                                <SidebarLink
                                    name="Armada"
                                    routeName="delivery.vehicles.index"
                                    icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10h1v-1a1 1 0 011-1h4a1 1 0 011 1v1h2zm4 0v-4l-3-3h-2v7h1v-1a1 1 0 011-1h2a1 1 0 011 1v1h1z" /></svg>}
                                />
                                <SidebarLink
                                    name="Pengaturan Sewa Pompa"
                                    routeName="delivery.pump-rental-settings.index"
                                ...
                                {/* Separator - Keranjang Sampah */}
                                <div className="pt-4 pb-2 px-3 transition-all duration-200 md:opacity-0 md:group-hover:opacity-100 overflow-hidden whitespace-nowrap">
                                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Keranjang Sampah</p>
                                </div>

                                <SidebarLink
                                    name="Sampah Proyek"
                                    routeName="delivery.projects.trash"
                                    icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>}
                                />
                                <SidebarLink
                                    name="Sampah Customer"
                                    routeName="delivery.customers.trash"
                                    icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>}
                                />
                            </>
                        )}
