...
export default function Show({ project }) {
  ...
  const [showImageModal, setShowImageModal] = useState(null) // Untuk popup lihat gambar struk

  const [selectedMonth, setSelectedMonth] = useState(new Date().toISOString(). slice (0, 7))

  // Hitung total pengeluaran di frontend (simple way)
  const totalExpense = project.expenses.reduce((acc, curr) => acc + parseFloat(curr.amount), 0)

  // Form handling untuk tambah pengeluaran
  const { data, setData, post, processing, errors, reset } = useForm({
    project_id: project.id,
    title: '',
    amount: '',
    transacted_at: new Date().toISOString().split('T')[0], // Default hari ini
    description: '',
    receipt_image: null,
  })
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <TextInput 
                        ...
                        placeholder="Misal: Beli Semen 50 Sak"
                        required
                    />
