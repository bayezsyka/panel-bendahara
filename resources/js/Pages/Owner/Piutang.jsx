...
const FilterBar = ({ search, setSearch, projectId, setProjectId, status, setStatus, allProjects, onApply, pdfHref, pdfLabel }) => {
    const [localSearch, setLocalSearch] = useState(search);
    const debounceRef = useRef(null);

    const handleSearchChange = (val) => {
        setLocalSearch(val);
        setSearch(val); // sync to parent
    };
            ...
            <div className="flex flex-wrap items-end gap-3">
                ...
                <div className="flex-1 min-w-[180px]">
                    ...
                    <div className="relative">
                        ...
                        />
                        {localSearch && (
                            <button onClick={() => { handleSearchChange(''); onApply(); }}
                ...
                <div className="min-w-[180px]">
                    <label className="block text-xs font-medium text-gray-500 mb-1">Pilih Proyek Spesifik</label>
                ...
                {pdfHref && (
                    <a href={projectId ? `${pdfHref}&project_id=${projectId}` : pdfHref}
        ...
        <OwnerLayout header={<PageHeader title="Piutang & Tagihan" />}>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-5">
                    {[
                        { label: 'Total Tagihan',       value: rupiah(summary.grand_tagihan), cls: 'text-gray-900',    bg: 'from-gray-50 to-white'    },
                        { label: 'Total Sudah Dibayar', value: rupiah(summary.grand_bayar),   cls: 'text-emerald-600', bg: 'from-emerald-50 to-white'  },
                        { label: 'Sisa Piutang',        value: rupiah(summary.grand_sisa),    cls: 'text-red-600',     bg: 'from-red-50 to-white'      },
                        { label: 'Proyek Belum Lunas',  value: summary.count_unpaid,          cls: 'text-orange-600',  bg: 'from-orange-50 to-white'   },
                        { label: 'Proyek Lunas',        value: summary.count_paid,            cls: 'text-indigo-600',  bg: 'from-indigo-50 to-white'   },
                    ].map(k => (
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <p className="text-sm font-bold text-indigo-600">{pctLunas}%</p>
                    </div>
                    <div className="h-3 rounded-full bg-gray-100 overflow-hidden">
                        <div className="h-full bg-indigo-600 rounded-full transition-all duration-700" style={{ width: `${pctLunas}%` }} />
                    </div>
                ...
                {paymentTrend.length > 0 && (
                    <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                        <h3 className="text-base font-semibold text-gray-900 mb-4">Tren Penerimaan Pembayaran (6 Bulan)</h3>
                ...
                <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
                    <div className="border-b border-gray-100 px-6 py-4 flex items-center justify-between">
                        <h3 className="text-base font-semibold text-gray-900">Status Piutang per Proyek</h3>
                        <span className="text-sm text-gray-500">{projects.length} proyek</span>
                    </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="text-right px-6 py-3 text-xs font-semibold text-gray-500">Total Tagihan</th>
                                    <th className="text-right px-6 py-3 text-xs font-semibold text-gray-500">Dibayar</th>
                                    <th className="text-right px-6 py-3 text-xs font-semibold text-gray-500">Sisa</th>
                                    <th className="px-6 py-3 text-xs font-semibold text-gray-500">Lunas</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {projects.length === 0 ? (
                                    ...
                                    <tr key={p.id} className="hover:bg-gray-50">
                                        <td className="px-6 py-3 font-medium text-gray-900 max-w-[180px] truncate">{p.project_name}</td>
                                        <td className="px-6 py-3 text-gray-600 max-w-[140px] truncate">{p.customer_name}</td>
                                        <td className="px-6 py-3 text-right tabular-nums text-gray-900">{rupiah(p.total_tagihan)}</td>
                                        <td className="px-6 py-3 text-right tabular-nums text-emerald-600">{rupiah(p.total_bayar)}</td>
                                        <td className="px-6 py-3 text-right tabular-nums font-semibold text-red-600">{p.sisa_piutang > 0 ? rupiah(p.sisa_piutang) : '—'}</td>
                                        <td className="px-6 py-3">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-gray-500 w-8 text-right">{p.pct_lunas}%</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-3">
                                            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${p.status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                ...
                <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
                    <div className="border-b border-gray-100 px-6 py-4 flex items-center justify-between">
                        <h3 className="text-base font-semibold text-gray-900">Riwayat Pembayaran</h3>
                        <span className="text-sm text-gray-500">{recentPayments.length} terakhir</span>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500">Proyek</th>
                                    <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500">Customer</th>
                                    <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500">Catatan</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {recentPayments.length === 0 ? (
                                    ...
                                    <tr key={p.id} className="hover:bg-gray-50">
                                        <td className="px-6 py-3 text-gray-500 whitespace-nowrap">{p.date}</td>
                                        <td className="px-6 py-3 font-medium text-gray-800 max-w-[160px] truncate">{p.project}</td>
                                        <td className="px-6 py-3 text-gray-600 max-w-[120px] truncate">{p.customer}</td>
                                        <td className="px-6 py-3 text-gray-400 max-w-[180px] truncate">{p.note}</td>
                                        <td className="px-6 py-3 text-right font-semibold text-emerald-600 tabular-nums">{rupiah(p.amount)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
